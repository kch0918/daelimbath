		...
		success : function(data) 
		{
			...
			if(result.list.length > 0)
			{
				for(var i = 0; i < result.list.length; i++)
				{
					...
					inner += '	<td><button type="button" class="small-btn btn_a_type edit-btn" onclick="javascript:location.href=\'./etc_upload?idx='+result.list[i].idx+'\'">수정</button><button type="button" class="small-btn btn_b_type delete-btn" onclick="delEtcList();">삭제</button></td>'
					inner += '</tr>';		
				}
			}
			else
...
function showStatusChange(num,status){
	...
	if(confirm("'" + bannerTitle + "' 노출 상태를 '" + changeResult + "'로 변경하시겠습니까?"))
	{
		$.ajax({
			type : "POST", 
			url : "/admin/board/changeExpo_etc",
			dataType : "text",
			async : false,
			data : 
			{
...
<div id="container">
	<div class="admin-top">
		<h1 class="admin-title">기타 자료실 리스트</h1>
		<div class="top-float">
			<a href="etc_upload" class="btn btn_a_type">기타 자료 등록</a>
		</div>
	</div>
