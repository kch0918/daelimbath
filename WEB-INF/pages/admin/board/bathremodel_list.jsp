...
function getList(paging_type) 
{
	...
	if(($("#show_y").is(":checked") && $("#show_y").is(":checked")) || (!$("#show_n").is(":checked") && !$("#show_y").is(":checked")))
	{
		list_expo_yn = "";
	}
	else
	...
	$.ajax({
		type : "POST", 
		url : "/admin/board/getBathRemodelList",
		dataType : "text",
		data : 
		{
			page 	 		:  page,
		...
		success : function(data) 
		{
			...
			if(result.list.length > 0)
			{
				for(var i = 0; i < result.list.length; i++)
				{
					...
					inner += '	</td>';
					inner += '	<td>'+[i+1]+'</td>';
					inner += '	<td><img src="/upload/bathremodel/'+result.list[i].thumb_ori+'" style="width:30px; height:30px;"></td>';
					inner += '	<td>'+nullChk(result.list[i].title)+'</td>';
					
					inner += '	<td>';
					// 리스트 노출여부
					if(result.list[i].list_expo_yn == "1")
					{
						inner += '	<div id="show_yn_'+result.list[i].idx+'" class="show-btn-wrap status-on">';
						inner += '		<button type="button" class="show-btn show-off" id="show_n_'+result.list[i].idx+'" onclick="list_showStatusChange('+result.list[i].idx+',\'n\')">비노출하기</button>';
						inner += '		<button type="button" class="show-btn show-on" id="show_y_'+result.list[i].idx+'" onclick="list_showStatusChange('+result.list[i].idx+',\'y\')" >노출하기</button>';
						inner += '	</div>';
					}
					else
					{
					...
					inner += '	<td>';
					// 인덱스 노출여부
					if(result.list[i].idx_expo_yn == "1")
					{
						inner += '	<div id="show_yn_'+result.list[i].idx+'" class="show-btn-wrap status-on">';
						inner += '		<button type="button" class="show-btn show-off" id="show_n_'+result.list[i].idx+'" onclick="idx_showStatusChange('+result.list[i].idx+',\'n\')">비노출하기</button>';
						inner += '		<button type="button" class="show-btn show-on" id="show_y_'+result.list[i].idx+'" onclick="idx_showStatusChange('+result.list[i].idx+',\'y\')" >노출하기</button>';
						inner += '	</div>';
					}
					else
					{
					...
					inner += '	<td><button type="button" class="small-btn btn_a_type edit-btn" onclick="javascript:location.href=\'./bathremodel_upload?idx='+result.list[i].idx+'\'">수정</button></td>'
					inner += '</tr>';		
				}
			}
			else
...
function delBathRemodelList()
{
	...
	if(confirm("정말 삭제하시겠습니까?"))
	{
		$.ajax({
			type : "POST", 
			url : "/admin/board/delBathRemodelList",
			dataType : "text",
			async : false,
			data : 
			{
...
function list_showStatusChange(num,status){
	var btnWrapper = document.getElementById('show_yn_' + num),
		bannerTitle = btnWrapper.closest('tr').querySelector('td:nth-child(4)').textContent,
		changeResult,
		act = "";
	...
	if(confirm("'" + bannerTitle + "' 노출 상태를 '" + changeResult + "'로 변경하시겠습니까?"))
	{
		$.ajax({
			type : "POST", 
			url : "/admin/board/changeExpo_bathremodel_list",
			dataType : "text",
			async : false,
			data : 
			{
...
function idx_showStatusChange(num,status){
	var btnWrapper = document.getElementById('show_yn_' + num),
		bannerTitle = btnWrapper.closest('tr').querySelector('td:nth-child(4)').textContent,
		changeResult,
		act = "";
	...
	if(confirm("'" + bannerTitle + "' 노출 상태를 '" + changeResult + "'로 변경하시겠습니까?"))
	{
		$.ajax({
			type : "POST", 
			url : "/admin/board/changeExpo_bathremodel_idx",
			dataType : "text",
			async : false,
			data : 
			{
...
<div id="container">
	<div class="admin-top">
		<h1 class="admin-title">욕실 리모델링 리스트</h1>
		<div class="top-float">
			<a href="bathremodel_upload" class="btn btn_a_type">등록</a>
		</div>
	</div>
        ...
        <div class="search_con_wrap">
            <div class="info_box">
                    <span class="serch_cont_txt"><strong>결과 <span id="searchNum"></span>개</strong> / 전체 <span id="totalNum"></span>개</span>
                   	<span class="dis-no"><img src="/img/es_check.png"><span class="es_select">선택한 게시글</span><a onclick="delBathRemodelList();" class="del_bt"><img src=""/>선택 삭제</a></span>
        	...
        	<div class="list-btn">
        		<button type="button" onclick="delBathRemodelList();" class="btn btn_c_type list-delete-btn">선택 삭제</button>
        	</div>
