		...
		success : function(data) 
		{
			...
			if(result.list.length > 0)
			{
				for(var i = 0; i < result.list.length; i++)
				{
					...
					inner += '	<td>'+[i+1]+'</td>';
					inner += '	<td>';
					if(result.list[i].cata_cate == 1)
					{
						inner += '바스';
					}
					else if(result.list[i].cata_cate == 2)
					{
						inner += '바스&키친'; 
					}
					...
					inner += '	<td><button type="button" class="small-btn btn_a_type edit-btn" onclick="javascript:location.href=\'./cata_upload?idx='+result.list[i].idx+'\'">수정</button><button type="button" class="small-btn btn_b_type delete-btn" onclick="delCataList();">삭제</button></td>'
					inner += '</tr>';		
				}
			}
			else
...
function showStatusChange(num,status){
	...
	if(confirm("'" + bannerTitle + "' 노출 상태를 '" + changeResult + "'로 변경하시겠습니까?"))
	{
		$.ajax({
			type : "POST", 
			url : "/admin/board/changeExpo_cata",
			dataType : "text",
			async : false,
			data : 
			{
...
<div id="container">
	<div class="admin-top">
		<h1 class="admin-title">카탈로그 리스트</h1>
		<div class="top-float">
			<a href="cata_upload" class="btn btn_a_type">카탈로그 등록</a>
		</div>
	</div>
	<div class="contain_inner">
        ...
        <div class="search_con_wrap">
        	...
        	<div class="list-btn">
        		<button type="button" onclick="delCataList();" class="btn btn_c_type list-delete-btn">선택 삭제</button>
        	</div>
