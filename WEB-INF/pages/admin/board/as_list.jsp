...
function getList(paging_type) 
{
	...
	$.ajax({
		type : "POST", 
		url : "/admin/board/getAsGuideList",
		dataType : "text",
		data : 
		{
			page 	 	:  page,
		...
		success : function(data) 
		{
			...
			if(result.list.length > 0)
			{
				for(var i = 0; i < result.list.length; i++)
				{
					...
					inner += '	</td>';
					inner += '	<td>'+[i+1]+'</td>';
					inner += '	<td><img src="/upload/asguide/'+result.list[i].thumb_ori+'" style="width:30px; height:30px;"></td>';
					...
					inner += '	<td><button type="button" class="small-btn btn_a_type edit-btn" onclick="javascript:location.href=\'./as_upload?idx='+result.list[i].idx+'\'">수정</button></td>'
					inner += '</tr>';		
				}
			}
			else
...
function delEtcList()
{
	...
	if(confirm("정말 삭제하시겠습니까?"))
	{
		$.ajax({
			type : "POST", 
			url : "/admin/board/delAsGuideList",
			dataType : "text",
			async : false,
			data : 
			{
...
function showStatusChange(num,status){
	...
	if(confirm("'" + bannerTitle + "' 노출 상태를 '" + changeResult + "'로 변경하시겠습니까?"))
	{
		$.ajax({
			type : "POST", 
			url : "/admin/board/changeExpo_asGuide",
			dataType : "text",
			async : false,
			data : 
			{
...
<div id="container">
	<div class="admin-top">
		<h1 class="admin-title">AS/설치 가이드 리스트</h1>
		<div class="top-float">
			<a href="as_upload" class="btn btn_a_type">등록</a>
		</div>
	</div>
	    		...
	    		<div class="search-box-wrap">
	    			...
	    			<div class="search-box">
	    				...
	    				<div class="search-right">
	    					<div class="search-input">
	                            <div class="check_box">
	                                <span class="input-chkbox" onclick=""><input type="checkbox" id="show_y" name="expo_yn" value="1" checked/><label for="show_y">노출</label></span>
	                                <span class="input-chkbox" onclick=""><input type="checkbox" id="show_n" name="expo_yn" value="2" checked/><label for="show_n">미노출</label></span>
	                             </div>
	    					</div>
	    				</div>
	    			</div>
