	...
	if(validationFlag == "Y")
	{
	      $("#fncForm").ajaxSubmit({
			success: function(data)
			{
	    		...
	    		{
	    			alert(result.msg);
	     			location.href = '/admin/main/notice_slide_list';
	    		}
	    		else
	    		{
	    			alert(result.msg);
...
function init() {
	...
	$("#fake_file_pc").val('${data.file_pc}');
	$("#fake_file_mo").val('${data.file_mo}');	
...
<div id="container">
	<div class="contain_inner">
		<div class="admin-top">
			<h1 class="admin-title">공지사항 슬라이드 배너 등록/수정</h1>
		</div>
		<form id="fncForm" name="fncForm" action="/admin/main/notice_slide_upload_proc" method="post" enctype="multipart/form-data">
				...
				<div class="input-table">
					<table>
		   				...
		   				<tbody>
						    ...
						    <tr>
						    	<th><label for="nbanner_pc">PC 배너 등록<span class="required-mark">*</span></label></th>
		                        <td>
		                        	<div class="input-file-box">
		                        		<button type="button" onclick="javascript:$('#file_pc').trigger('click');" class="mid-btn btn_a_type">찾아보기</button>
		                        		<span class="input-file">
					                       	<input type="text" id="fake_file_pc" class="fake-upload notEmpty" placeholder="첨부파일"  data-name="PC 메인배너 등록 이미지"/>
				                            <input type="file" id="file_pc" name="file_pc" onchange="checkFile(this)"/>
		                        		</span>
		                        	</div>
			                    </td>
							</tr>
						    <tr>
						    	<th>모바일 배너 등록<span class="required-mark">*</span></th>
		                        <td class="file_add">
		                        	<div class="input-file-box">
		                        		<button type="button" onclick="trigger('#file_mo').trigger('click');" class="mid-btn btn_a_type list_length">찾아보기</button>
		                        		<span class="input-file">
					                       	<input type="text" id="fake_file_mo" class="imgNm fake-upload" placeholder="첨부파일"  data-name="모바일 메인배너 등록 이미지"/>
				                            <input type="file" id="file_mo" name="file_mo" onchange="checkFile(this)"/>
		                        		</span>
		                        	</div>
			                    </td>
							</tr>
						    <tr>
		                        ...
		                        <td>
			                        ...
			                        <input type="hidden" id="blank_val" name="blank"/>
			                        <select id="blank" class="custom-select">
