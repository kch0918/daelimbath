		...
		success : function(data) 
		{
			...
			if(result.list.length > 0)
			{
				for(var i = 0; i < result.list.length; i++)
				{
					...
					inner += '	<td><button type="button" class="small-btn btn_a_type edit-btn" onclick="javascript:location.href=\'./best_pro_upload?idx='+result.list[i].idx+'\'">수정</button><button type="button" class="small-btn btn_b_type delete-btn" onclick="delBestList();">삭제</button></td>'
					inner += '</tr>';		
				}
			}
			else
...
function showStatusChange(num,status){
	...
	if(confirm("'" + bannerTitle + "' 배너의 노출 상태를 '" + changeResult + "'로 변경하시겠습니까?"))
	{
		$.ajax({
			type : "POST", 
			url : "/admin/main/changeExpo_best",
			dataType : "text",
			async : false,
			data : 
			{
...
<div id="container">
	<div class="admin-top">
		<h1 class="admin-title">베스트 제품 리스트</h1>
		<div class="top-float">
			<a href="best_pro_upload" class="btn btn_a_type">제품 등록</a>
		</div>
	</div>
	    		...
	    		<div class="search-box-wrap">
	    				...
	    				<div class="search-left">
	                        ...
	                        <div class="search-input">
	                            <div class="date-select">
	                                <div class="date_box"><input type="text" id="start_date" name="" class="datepicker input-calendar" autocomplete="off" placeholder="0000-00-00"/></div>
	                                <div class="date_box period-bar">~</div>
	                                <div class="date_box"><input type="text" id="finish_date" name="" class="datepicker input-calendar" autocomplete="off" placeholder="0000-00-00"/></div>
	                            </div>
	                         </div>
                         </div>
	    			...
	    			<div class="search-box">
	    				...
	    				<div class="search-right">
	    					<div class="search-input">
	                            <div class="check_box">
	                                <span class="input-chkbox" onclick=""><input type="checkbox" id="show_y" name="show_y" value="" checked/><label for="show_y">노출</label></span>
	                                <span class="input-chkbox" onclick=""><input type="checkbox" id="show_n" name="show_n" value="" checked/><label for="show_n">미노출</label></span>
	                             </div>
	    					</div>
	    				</div>
	    			</div>
        ...
        <div class="search_con_wrap">
            <div class="info_box">
                    <span class="serch_cont_txt"><strong>결과 <span id="searchNum"></span>개</strong> / 전체 <span id="totalNum"></span>개</span>
                   	<span class="dis-no"><img src="/img/es_check.png"><span class="es_select">선택한 게시글</span><a onclick="delBestList();" class="del_bt"><img src=""/>선택 삭제</a></span>
        	...
        	<div class="list-btn">
        		<button type="button" onclick="deBestList();" class="btn btn_c_type list-delete-btn">선택 삭제</button>
        	</div>
