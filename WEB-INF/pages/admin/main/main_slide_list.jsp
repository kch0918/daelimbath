...
function getList(paging_type) 
{
	...
	$.ajax({
		type : "POST", 
		url : "/admin/main/getMainBannerList",
		dataType : "text",
		data : 
		{
			page 	 	 :  page,
		...
		success : function(data) 
		{
			...
			if(result.list.length > 0)
			{
				for(var i = 0; i < result.list.length; i++)
				{
					...
					inner += '	<td><button type="button" class="small-btn btn_a_type edit-btn" onclick="javascript:location.href=\'./main_slide_upload?idx='+result.list[i].idx+'\'">수정</button><button type="button" class="small-btn btn_b_type delete-btn" onclick="delMainBannerList();">삭제</button></td>'
					inner += '</tr>';		
				}
			}
			else
...
function delMainBannerList()
{
	...
	if(confirm("정말 삭제하시겠습니까?"))
	{
		$.ajax({
			type : "POST", 
			url : "/admin/main/delMainBannerList",
			dataType : "text",
			async : false,
			data : 
			{
...
<div id="container">
	<div class="admin-top">
		<h1 class="admin-title">메인슬라이드 배너 리스트</h1>
		<div class="top-float">
			<a href="/admin/main/main_slide_upload" class="btn btn_a_type">슬라이드 배너 등록</a>
		</div>
	</div>
	<div class="contain_inner">
        ...
        <div class="search_con_wrap">
        	...
        	<div class="list-btn">
        		<button type="button" onclick="delMainBannerList();" class="btn btn_c_type list-delete-btn">선택 삭제</button>
        	</div>
