		...
		success : function(data) 
		{
			...
			if(result.list.length > 0)
			{
				for(var i = 0; i < result.list.length; i++)
				{
					...
					if(result.list[i].start_date == "" || result.list[i].finish_date == "" )
					{
						inner += '	<td>상시</td>';
					}
					else
					{
						inner += '	<td>'+cutDate(result.list[i].start_date)+' ~ '+cutDate(result.list[i].finish_date)+'</td>';
					}
					inner += '<td>';
					...
					inner += '	<td><button type="button" class="small-btn btn_a_type edit-btn" onclick="javascript:location.href=\'./notice_slide_upload?idx='+result.list[i].idx+'\'">수정</button><button type="button" class="small-btn btn_b_type delete-btn" onclick="delNoticeList();">삭제</button></td>'
					inner += '</tr>';		
				}
			}
			else
...
function showStatusChange(num,status){
	...
	if(confirm("'" + bannerTitle + "' 배너의 노출 상태를 '" + changeResult + "'로 변경하시겠습니까?"))
	{
		$.ajax({
			type : "POST", 
			url : "/admin/main/changeExpo_notice",
			dataType : "text",
			async : false,
			data : 
			{
...
<div id="container">
	<div class="admin-top">
		<h1 class="admin-title">공지사항 슬라이드 배너 리스트</h1>
		<div class="top-float">
			<a href="/admin/main/notice_slide_upload" class="btn btn_a_type">슬라이드 배너 등록</a>
		</div>
	</div>
        ...
        <div class="search_con_wrap">
            <div class="info_box">
                    <span class="serch_cont_txt"><strong>결과 <span id="searchNum"></span>개</strong> / 전체 <span id="totalNum"></span>개</span>
                   	<span class="dis-no"><img src="/img/es_check.png"><span class="es_select">선택한 게시글</span><a onclick="delNoticeList();" class="del_bt"><img src=""/>선택 삭제</a></span>
        	...
        	<div class="list-btn">
        		<button type="button" onclick="delNoticeList();" class="btn btn_c_type list-delete-btn">선택 삭제</button>
        	</div>
