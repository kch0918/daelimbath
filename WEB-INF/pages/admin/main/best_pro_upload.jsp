...
function fncSubmit()
{
	...
	var pro_key = "";
	//배열 생성
	var grparr = new Array(pro_key);
	//배열에 값 주입
	for(var i=0; i < 3; i++){                          
		grparr[i] = $("input[name=keyword]").eq(i).val();
		pro_key += grparr[i] + '|';
	}
	...
	if(validationFlag == "Y")
	{
	      $("#fncForm").ajaxSubmit({
			success: function(data)
			{
	    		...
	    		{
	    			alert(result.msg);
	     			location.href = '/admin/main/best_pro_list';
	    		}
	    		else
	    		{
	    			alert(result.msg);
...
function init() {
	...
	$("#pro_nm").val(repWord(nullChk('${data.pro_nm}')));
	// 제품특징 
	$("#keyword1").val('${data.pro_key1}');
	$("#keyword2").val('${data.pro_key2}');
	$("#keyword3").val('${data.pro_key3}');
	...
	$("#fake_pro_img_mo").val('${data.pro_img_mo}');	
...
<div id="container">
	<div class="contain_inner">
		<div class="admin-top">
			<h1 class="admin-title">베스트 제품 등록/수정</h1>
		</div>
		<form id="fncForm" name="fncForm" action="/admin/main/best_pro_upload_proc" method="post" enctype="multipart/form-data">
		    <div class="insert-form-box">
				<div class="input-table">
					<table>
		   				...
		   				<tbody>
							<tr>
								<th><label for="best_product_nm">제품명<span class="required-mark">*</span></label></td>
			                    <td>
			                        <input type="text" class="info_area notEmpty" id="pro_nm" name="pro_nm" placeholder="제품명을 입력해주세요 " data-name="제품명"/>
			                    </td>
							</tr>
							<tr>
								<th><label for="best_keyword_1">키워드<span class="required-mark">*</span></label></td>
			                    <td>
			                    	<div class="keyword-input">
				                        <input type="text" class="info_area" id="keyword1" name="pro_key1" placeholder="키워드 1"/>
				                        <input type="text" class="info_area" id="keyword2" name="pro_key2" placeholder="키워드 2"/>
				                        <input type="text" class="info_area" id="keyword3" name="pro_key3" placeholder="키워드 3"/>
			                    	</div>
			                    </td>
							</tr>
						    <tr>
						    	<th>메인 노출 여부<span class="required-mark">*</span></th>
		                        <td>
						    ...
						    <tr>
						    	<th><label for="pro_img">PC 이미지 등록<span class="required-mark">*</span></label></th>
		                        <td>
		                        	<div class="input-file-box">
		                        		<button type="button" onclick="javascript:$('#pro_img').trigger('click');" class="mid-btn btn_a_type">찾아보기</button>
		                        		<span class="input-file">
					                       	<input type="text" id="fake_pro_img" class="fake-upload notEmpty" placeholder="첨부파일"  data-name="PC 이미지 등록 이미지"/>
						    ...
						    <tr>
						    	<th><label for="pro_img_mo">모바일 메인배너 등록<span class="required-mark">*</span></label></th>
		                        <td class="file_add">
		                        	<div class="input-file-box">
		                        		<button type="button" onclick="trigger('#pro_img_mo').trigger('click');" class="mid-btn btn_a_type list_length">찾아보기</button>
		                        		<span class="input-file">
					                       	<input type="text" id="fake_pro_img_mo" class="imgNm fake-upload" placeholder="첨부파일"  data-name="모바일 이미지 등록 이미지"/>
				                            <input type="file" id="pro_img_mo" name="pro_img_mo" onchange="checkFile(this)"/>
		                        		</span>
		                        	</div>
			                    </td>
							</tr>
						    <tr>
						    	<th>제품 연결 링크<span class="required-mark">*</span></th>
		                        <td>
			                        <input type="text" class="info_area notEmpty" id="link" name="link" placeholder="제품과 연결할 링크를 입력해주세요 "/>
